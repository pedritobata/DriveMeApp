...
export default class DriverTripComplete extends React.Component {
                    ...
                    path.once('value',snapVal=>{
                      if(snapVal.val()){
                         ...
                         if(fRating){
                             ...
                             firebase.database().ref('users/'+this.state.getDetails.driver+'/ratings/').update({userrating:fRating.toFixed(1)}).then(()=>{
    ...
    render(){
        ...
        <Text style={styles.rateViewTextStyle}>{Currency}{this.state.getDetails?this.state.getDetails.trip_cost>0?this.state.getDetails.trip_cost.toFixed(2):0:null}</Text>
                </View>
